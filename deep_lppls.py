...
## This Part is for deep-LPPLS
# LPPLS 함수 정의
def lppls(t, tc, m, omega, A, B, C1, C2):
    dt = tc - t
    return A + B * np.power(dt, m) * (1 + C1 * np.cos(omega * np.log(dt)) + C2 * np.sin(omega * np.log(dt)))

# 패널티 함수 정의
def penalty_function(theta, theta_min, theta_max):
    return K.sum(K.maximum(0., theta_min - theta) + K.maximum(0., theta - theta_max))
